...
module.exports = {
    getBudget: async (req,res)=>{
        try{
            ...
            let reqDate
            if(!req.body.month){
                thisBudget = await Budget.find({userId:req.user.id, date: req.params.date })
                reqDate = req.params.date
            }else{
                thisBudget = await Budget.find({userId:req.user.id, date: req.body.month })
                 reqDate = req.body.month
            ...
            res.render("budget.ejs", {budget: thisBudget[thisBudget.length-1], user: req.user, left: moneyLeft, reqDate: reqDate, moment: moment })
        }catch(err){
            console.log(err)
        }
    },
